        ...
        <div className='flex w-screen flex-col align-middle items-center bg-teal-100'>
            ...
            <div className="content flex flex-col w-9/12 gap-8 mt-6 mb-12">
                <div className="shop-info flex flex-row justify-between p-8 rounded-2xl bg-white shadow-[5px_5px_20px_2px_rgba(0,0,0,0.25)]">
                    <div className="info-l flex flex-col gap-5 text-teal-500 h-full ">
                        <div className='text-black mb-3 font-bold text-2xl'> { shop.name }</div>
                        <div className='flex flex-row gap-2 items-center  '>
                            <div>
                                <FaMapMarkerAlt />
                            </div>
                            <div class='flex'>
                                <p class='break-all' >{ shop.location }</p>
                            </div>
                        </div>
                        <div className='flex flex-row gap-2 items-center max-w-max '><FaGlobe /><p class='break-all'>{ shop.city }</p></div>
                        <div className='flex flex-row gap-2 items-center max-w-max '><FaEnvelope /><p class='break-all'>{ shop.email }</p></div>
                        <div className='flex flex-row gap-2 items-center max-w-max'><FaPhone /><p class='break-all'>{ shop.contact }</p></div>
                    </div>
                    <div className=" info-r ">
                        <div className="hidden md:flex  verify  w-28 h-8 items-center justify-center rounded-2xl bg-teal-500 text-white">PT Verified</div>
                    </div>
                </div>
